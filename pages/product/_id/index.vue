<template>
    <div>           
        <v-row class="mt-4">
            <h1 class="ml-4">{{ isEditing ? 'Edição de produto' : 'Cadastro de produto' }}</h1>
        </v-row>
        <v-card
...
export default Vue.extend({
  ...
  methods: {
    ...
    async save(){
        ...
        {            
            await this.$axios.post<number>('/product', this.fields)
            .then(() => {
    ...
    async edit(id: number){
        await this.$axios.put('/product/' + id, this.fields)
            .then(() => {
                this.$globalFunctions.successAlert('Produto salvo com sucesso', 5000)
                this.leave()                
            })
