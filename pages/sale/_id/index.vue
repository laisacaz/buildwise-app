<template>
    <div>
        ...
        >
                <v-card class="no-scroll" >
                    ...
                    <v-row no-gutters>
                        ...
                        <v-col cols="auto">
                            <v-currency-field v-model="fields.increase" 
                                outlined
                                dense
                                style="max-width: 80px"
                                disabled></v-currency-field>
                        </v-col>
                    </v-row>
                    ...
                    <v-row class="ml-2">  
                        ...
                        <v-col cols="auto">
                            ...
                            v-model="fields.paidAmount"
                             ...
                             disabled
                             dense
                             style="max-width: 80px"></v-currency-field>
                        </v-col>
                    ...
                    <v-row no-gutters class="mt-4">
                        ...
                        <v-col cols="auto" class="ml-2">
                            <v-currency-field                             
                                ...
                                @input="calculateDiscountAndIncrease"></v-currency-field>
                        </v-col>
                    </v-row>  
                    ...
                    <v-row class="ml-2">  
                        ...
                        <v-col cols="auto">
                            ...
                            v-model="fields.paidAmount"
                             ...
                             dense
                             style="max-width: 80px"
                             @change="checkIfHasMoneyToReturn"></v-currency-field>
                        </v-col>
                        <v-col cols="auto" v-if="moneyToReturn">
                            <h4 class="mt-2">Troco</h4>
                        </v-col>
        ...
        <v-form v-if="isSearchingConstruction">
            <v-dialog
                v-model="isSearchingConstruction"
                persistent
                max-width="800px"
        >
                <v-card class="no-scroll" >
                        ...
                        <template  #[`item.select`]="{ item }">
                            <v-tooltip bottom>
                                <template #activator="{ on, attrs }">
                                    ...
                                    >
                                    <v-icon>   {{ item.selectedBtn == true ? 'mdi-check' : 'mdi-plus' }}    </v-icon>
                                    {{ item.selectedBtn == true ? 'Selecionado' : 'Selecionar' }}    
                                    </v-btn>
                                </template>
                            <span>{{ item.selectedBtn == true ? 'Selecionado' : 'Selecionar'}}</span>
                            </v-tooltip>
                        </template> 
        ...
        <v-row class="mt-4">
            <h1 class="ml-4">{{ isEditing && !isConsulting ? 'Edição de venda' : 
            isConsulting === true ? 'Consulta de venda' : 'Cadastro de venda' }}</h1>
        </v-row>
        <v-card
        elevated
        class="mt-6 mb-6"> 
            ...
            <v-col>
                <v-tooltip bottom>
                    <template #activator="{ on, attrs }">
                        ...
                        v-bind="attrs"
                        v-on="on"
                        :color="fields.status === EStatusSale.Open 
                        ? 'green white--text' : 'primary'"
                        elevated
                        style="height: 40px"
                        label>
            ...
            class="mb-6">
                <v-row class="ml-2 mt-2">
                    ...
                    <v-row justify="end" class="ml-2 mt-2 mb-2">
                        ...
                        <v-col v-if="isConsulting" cols="auto"
                        class="mr-6"
                        ></v-col>
...
import Vue from 'vue'
import { EConstructionSearchType, EProductSearchType,
     ESaleReceivementMethod, EStatusSale } from '~/utils/enumerators/enum';
import { IConstructionSearchParams, IConstructionSearchResponse,
     IFinishSale,
     IProductSale, IProductSearchParams, IProductSearchResponse, ISale, ISaleById, ISimplePerson, ISimpleProduct, defaultSearchResponse } from '~/utils/interfaces/crudObjects';
...
export default Vue.extend({
  data() {
    return {
        ...
        fieldsConstruction: {} as defaultSearchResponse<IConstructionSearchResponse>,
        headersConstruction: [
        {
            text: 'Código',
            value: 'id',
        ...
        { text: ' ', value: 'select', sortable: false, width: 80, align: 'center' },
        ],
        filtersProduct: {
            search: "",
        ...
        fieldsProducts: {                
        } as defaultSearchResponse<IProductSearchResponse>,
        headersProducts: [
            {
                text: 'Referência',
                value: 'reference',
  ...
  methods: {
    ...
    prepareUpdateReponse(){
        ...
        const editedProducts: ISimpleProduct[] = this.resultSale.products.map(x => ({
        ...
        if(this.resultSale.construction){
            ...
            this.constructionItem.clientName = this.resultSale.construction.client.name     
        }       
        }
        this.productsToBeInsertedInRegister = this.resultSale.products
    },
    async getActivesClient(){
        await this.$axios.get<ISimplePerson[]>('/person/actives/simple',)            
            .then((response) => {
    ...
    async getAllClient(){
        await this.$axios.get<ISimplePerson[]>('/person/simple',)            
            .then((response) => {
                this.items = response.data
            })
            .catch((error) => {
    ...
    async save(){     
        ...
        {                    
            ...
            await this.$axios.post<number>('/sale', this.fields)
            .then((response) => {                
                this.saleId = response.data 
                if(!this.isFinishing){
                    this.$globalFunctions.successAlert('Venda foi salva com sucesso', 5000)
                    this.leave()          
            }else{
                this.finishSale()
            }         
    ...
    async edit(id: number){
        await this.$axios.put('/sale/' + id, this.fields)
            .then(() => {  
                if(!this.isFinishing){ 
                    this.$globalFunctions.successAlert('Venda foi salva com sucesso', 5000)             
                    this.leave()
    ...
    async searchConstruction(){
        ...
        if(this.filtersConstruction.search){
          this.filtersConstruction.id = parseInt(this.filtersConstruction.search)
        }else{
        ...
        await this.$axios
        .get<defaultSearchResponse<IConstructionSearchResponse>>('/construction/search', {
          params: {
    ...
    selectConstructionItem(item: any){
        if(!item.selectedBtn){                           
           // verifica se tem algum outro botao selecionado  
            const selected =  this.fieldsConstruction.data.find((x) => x.selectedBtn === true)
    ...
    selectProductItem(item: any){
        if(!item.selectedBtn){  
            ...
            const selected =  this.fieldsProducts.data.find((x) => x.selectedBtn === true)
            ...
            if(!selected){
                ...
                this.productToBeInsertedInRegister = {
                    ...
                    productId: item.id,
                    price: item.price,
                    stockQuantitySale:  this.productToBeInsertedInRegister.stockQuantitySale
                }        
            }           
        }else{
            item.selectedBtn = !item.selectedBtn
    ...
    addProduct(){
        ...
        if(selected){
            if(this.productToBeInsertedInRegister.stockQuantitySale <= 0){
                this.$globalFunctions.attentionAlert('Quantidade do produto deve ser maior que 0', 5000)
            }else{
            //verifica se o item ja foi inserido no data table
            const productAlreadyInserted = this.productsToBeInsertedInRegister.find((x) => 
            ...
            if(productAlreadyInserted){
                 //procura index
                 const index = this.productsToBeInsertedInRegister.findIndex((x) => 
                x.productId === this.productToBeInsertedInRegister.productId)
                //adiciona quantidade
                ...
                // substitui                
                this.replaceProductsAlreadyAddedArray  = this.productsToBeInsertedInRegister
                this.productsToBeInsertedInRegister = this.replaceProductsAlreadyAddedArray
            }else{
            this.productsToBeInsertedInRegister.push(this.productToBeInsertedInRegister)
            }
            const productsToInsert: ISimpleProduct[] = this.productsToBeInsertedInRegister.map(x => ({
                productId: x.productId,
                stockQuantity: x.stockQuantitySale,
            }));                        
            // adiciona na venda
            ...
            this.$globalFunctions.successAlert('Produto adicionado com sucesso', 5000)               
            this.isAddingProduct = false   
        }                                  
        }else{
            this.$globalFunctions.attentionAlert('Nenhum produto selecionado', 5000)
        }
    ...
    const totalWithDiscountAndIncrease = totalWithIncrease - this.fields.discount!
    ...
    deleteProduct(item: IProductSale){
        const index = this.productsToBeInsertedInRegister.findIndex((x) => x.productId === item.productId)

        if (index !== -1) {
            this.productsToBeInsertedInRegister.splice(index, 1)
        }
        const indexArrayTwo = this.fields.products.findIndex((x) => x.productId === item.productId)
       
        if (index !== -1) {
            this.fields.products.splice(indexArrayTwo, 1)
            this.fields.subtotal = this.fields.subtotal - (item.stockQuantitySale * item.price)  
            this.fields.total = this.fields.subtotal           
        }        
        this.calculateTotalPriceProducts()
    },
    ...
    async finishSale(){
        ...
        await this.$axios.put('/sale/finish/' + this.saleId, finishSaleData)
        .then(() => {                            
            this.isFinishing = false
            this.$globalFunctions.successAlert('Venda finalizada com sucesso', 5000)                
            this.leave()   
        })
        .catch((error) => {            
            console.log(error)
    ...
    checkIfHasMoneyToReturn(){
        if(this.fields.receivementMethod == ESaleReceivementMethod.Money
        && this.fields.paidAmount! > 0 && this.fields.total! > 0){
