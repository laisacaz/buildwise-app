        ...
        <template  #[`item.edit`]="{ item }">
            <v-tooltip bottom>
              <template #activator="{ on, attrs }">
                ...
                >
                  {{ item.status == EStatusConstruction.Open ? 'mdi-pencil' : 'mdi-magnify' }}
                </v-icon>
              </template>
              <span>{{  item.status == EStatusConstruction.Open ? 'Editar' : 'Consultar' }}</span>
            </v-tooltip>
        </template>
...
import Vue from 'vue'
import { EConstructionSearchType, EStatusConstruction } from '~/utils/enumerators/enum';
import { IConstructionSearchParams, IConstructionSearchResponse, defaultSearchResponse } from '~/utils/interfaces/crudObjects';
  ...
  data() {
    return {
        ...
        headers: [
            ...
            { text: ' ', value: 'edit', sortable: false, width: 65, align: 'center' },
            { text: ' ', value: 'delete', sortable: false, width: 65, align: 'center'  }
        ],
        filters: {
            search: "",
