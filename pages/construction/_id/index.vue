<template>
    <div>
        ...
        <v-row class="mt-4">
            <h1 class="ml-4">{{ isEditing && !isConsulting ? 'Edição de obra' : 
            isConsulting === true ? 'Consulta de obra' : 'Cadastro de obra' }}</h1>
        </v-row>
...
import { IAddress, IConstruction, ISimplePerson } from '~/utils/interfaces/crudObjects';
  ...
  methods: {
    ...
    async save(){
        ...
        {                    
            ...
            await this.$axios.post<number>('/construction', this.fields)
            .then((response) => {                
                this.constructionId = response.data    
                this.$globalFunctions.successAlert('Construção salva com sucesso', 5000)     
                if(!this.isFinishing){
                    this.leave()
                }           
            })
    ...
    async edit(id: number){
        await this.$axios.put('/construction/' + id, this.fields)
            .then(() => {         
                this.$globalFunctions.successAlert('Construção salva com sucesso', 5000)    
                if(!this.isFinishing){   
                    this.leave()          
    ...
    async finishConstruction(){
        ...
        await this.$axios.put('/construction/finish/' + this.constructionId)
            .then(() => {        
