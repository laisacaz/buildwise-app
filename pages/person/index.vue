<template>    
  <div>    
    ...
    <v-row class="mt-4">
      <h1 class="ml-4">Pesquisa de pessoas</h1>
    </v-row>
...
import { IPersonSearchParams, IPersonSearchResponse, IProductSearchResponse, defaultSearchResponse } from '~/utils/interfaces/crudObjects';
...
methods: {
   async search(){
    ...
    await this.$axios.get<defaultSearchResponse<IPersonSearchResponse>>('/person/search', {
    params: {
      id: this.filters.id,
  ...
  .catch((error) => {
    ...
    async deletePerson(){
      await this.$axios.put('/person/delete/' + this.itemIdToDelete, {    
        headers: { 'content-type': 'application/json'}
      })
    .then(() => {
