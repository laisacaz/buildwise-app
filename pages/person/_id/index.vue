<template>
    <div>       
        <v-row class="mt-4">
            <h1 class="ml-4">{{ isEditing ? 'Edição de pessoa' : 'Cadastro de pessoa' }}</h1>
        </v-row>
  ...
  methods: {
    ...
    async save(){
        if(this.isEditing)
        {
            ...
            await this.$axios.post<number>('/person', this.fields)
            .then((response) => {
    ...
    async edit(id: number){
        await this.$axios.put('/person/' + id, this.fields)
            .then(() => {
                this.$globalFunctions.successAlert('Pessoa salva com sucesso', 5000)
                this.leave()                
            })
